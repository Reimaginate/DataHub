...
public class GetMergeFailuresByIdRequestHandler(IMediator mediator, IMapper mapper) : IHandler<GetMergeFailuresByIdRequest, GetMergeFailuresResponse>
{
    public async Task<GetMergeFailuresResponse> HandleAsync(GetMergeFailuresByIdRequest request, CancellationToken cancellationToken)
    {
        var parameters = new List<QueryParameter>
        {
            new("type", nameof(MergeFailure))
        };
