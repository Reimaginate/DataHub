...
public class DeleteJobsRequestHandler(IMediator mediator) : IHandler<DeleteJobsRequest, DeleteJobsResponse>
{
    public async Task<DeleteJobsResponse> HandleAsync(DeleteJobsRequest request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            while (getJobsResponse.MoreResultsAvailable)
            {
                getJobsRequest.ContinuationToken = getJobsResponse.ContinuationToken;
                ...
                jobsIdsToDelete = getJobsResponse.Results.Select(s => s.id).ToList();
                deleteResponse = (await mediator.TrySend(new ProcessDeleteJobsRequest()
                {
                    JobIds = jobsIdsToDelete
                }, cancellationToken)) switch { { Item2: { } exception } => throw exception, { Item1: var mediatorResultValue } => mediatorResultValue };
