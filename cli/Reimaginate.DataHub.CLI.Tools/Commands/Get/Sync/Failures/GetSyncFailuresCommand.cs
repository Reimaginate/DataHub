...
public class GetSyncFailuresCommand : SubCommand<GetSyncCommand>
{
    public GetSyncFailuresCommand(IServiceProvider serviceProvider) : base("failures", "Retrieve sync failures from the Data Hub", serviceProvider)
    {
    ...
    public async Task<int> HandleCommand(string? properties = null, string? additionalProperties = null, int? pageSize = 100, bool info = false, string saveTo = "", bool dontOpen = false, CancellationToken cancellationToken = default)
    {
        ...
        var baseReq = new GetSyncFailuresWhereRequest();
   
        var resultsFunc = () => CliHelpers.RetrieveAllResultsAsync<GetSyncFailuresWhereRequest, GetSyncFailuresResponse, SyncFailureDTO>(ServiceProvider, baseReq, r => r.Results, r => r.MoreResultsAvailable, (req, res) => req.ContinuationToken = res.ContinuationToken);
