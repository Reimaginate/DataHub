...
public class GetSyncFailuresByIdCommand : SubCommand<GetSyncFailuresCommand>
{
    public GetSyncFailuresByIdCommand(IServiceProvider serviceProvider) : base("byid", serviceProvider)
    {
    ...
    public async Task<int> HandleCommand(string[] ids, string? properties = null, string? additionalProperties = null, bool info = false, string saveTo = "", bool dontOpen = false)
    {
        ...
        var baseReq = new GetSyncFailuresByIdRequest()
        {
            Ids = ids.ToList()
        };

        var resultsFunc = () => CliHelpers.RetrieveAllResultsAsync<GetSyncFailuresByIdRequest, GetSyncFailuresResponse, SyncFailureDTO>(ServiceProvider, baseReq, r => r.Results, r => false);
